...
import {
  ExternalLink, X, Upload, FileText,
...
interface Props {
  jobTitle: string;
  company: string;
  /** "top"  → renders only the small "Apply" button (dispatches event to open the modal)
   *  "bottom" → renders the wide "Apply for this position" button + owns the modal */
  slot: "top" | "bottom";
}

type Step = "form" | "submitting" | "success";

const EVENT = "greatify:open-apply";

/* ─────────────────────────────────────────────────────────────────────────────
   TOP SLOT  — just a button that fires a custom event
───────────────────────────────────────────────────────────────────────────── */
function TopButton() {
  return (
    <Button
...
/* ─────────────────────────────────────────────────────────────────────────────
   BOTTOM SLOT  — button + full modal (listens for the custom event too)
───────────────────────────────────────────────────────────────────────────── */
function BottomButton({ jobTitle, company }: { jobTitle: string; company: string }) {
  ...
  // Listen for the top button's custom event
  useEffect(() => {
    window.addEventListener(EVENT, openModal);
    return () => window.removeEventListener(EVENT, openModal);
  }, [openModal]);
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium" htmlFor="apply-name">Full name</label>
                      <input
                        id="apply-name"
                        required
                        value={name}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium" htmlFor="apply-email">Email address</label>
                      <input
                        id="apply-email"
                        type="email"
                        required
              ...
              {step === "success" && (
                <div className="flex flex-col items-center gap-4 py-8 text-center">
                  ...
                  <Sparkles
                    ...
                    style={{ animation: "fadeIn 0.6s 0.4s both" }}
                  />
                  ...
                  <div style={{ animation: "fadeIn 0.5s 0.2s both" }}>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      ...
                      <span className="font-medium text-foreground">{company}</span> submitted.
                    </p>
...
/* ─────────────────────────────────────────────────────────────────────────────
   PUBLIC EXPORT — dispatches to the right sub-component
───────────────────────────────────────────────────────────────────────────── */
export function JobDetailClient({ jobTitle, company, slot }: Props) {
  if (slot === "top") return <TopButton />;
  return <BottomButton jobTitle={jobTitle} company={company} />;
}
